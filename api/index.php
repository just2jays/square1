 ...
 *-------------------------*/
    ...
    public function fetchInventory($userid) {
        $itemArray = array();
        foreach ($this->db->query("SELECT i.item_name, i.item_image_location, ui.unique_id, ui.timestamp FROM user_item ui INNER JOIN item i ON ui.item_id = item.id WHERE ui.user_id = '.$userid.' ORDER BY ui.timestamp DESC LIMIT 20;") as $row) {
            $itemArray[] = $row;
        }
 ...
 *-------------------------*/
    ...
    public function getCheckinCollection() {
        ...
        foreach ($this->db->query("SELECT checkin.id as 'checkin_id', checkin.checkin_latitude, checkin.checkin_longitude, checkin.checkin_review, checkin.checkin_timestamp, place.place_name FROM checkin INNER JOIN place ON checkin.checkin_place_id = place.id WHERE checkin.checkin_user_id = ".$userid." ORDER BY checkin.id DESC LIMIT 10") as $row) {
            $response[] = array(
                'ID' => $row['checkin_id'],
                'name' => $row['place_name'],
